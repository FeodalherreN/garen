...
use crate::models::errors::{
    FailedToOpenLockfileError, LockFileError, LockfileErrorTypes, RiftInitializationError,
};
...
lazy_static! {
    static ref REMOTING_AUTH_TOKEN: Regex = Regex::new(lockfile::REMOTING_AUTH_TOKEN).unwrap();
    static ref APP_PORT: Regex = Regex::new(lockfile::APP_PORT).unwrap();
}
...
fn find_lockfile_location() -> Result<String, RiftInitializationError> {
    ...
    if !stdout.next().unwrap_or("").starts_with("ExecutablePath") {
        return Err(RiftInitializationError::GenericLockfile(
            LockFileError::new(LockfileErrorTypes::LeagueNotRunning),
        ));
    }
...
pub fn get_lockfile() -> Result<LockfileContents, RiftInitializationError> {
    if cfg!(windows) {
        ...
        return match location {
            Ok(l) => {
                let mut file_path = l
                    .replace("LeagueClientUx.exe", "lockfile")
                    .replace("\r", "")
                    .replace("\\", "\\\\");
                ...
                match file {
                    Ok(f) => {
                        ...
                        if split.len() != 5 {
                            return Err(RiftInitializationError::GenericLockfile(
                                LockFileError::new(LockfileErrorTypes::LockfileUnexpectedFormat),
                            ));
                        }
                        Ok(LockfileContents {
                    ...
                    Err(e) => Err(RiftInitializationError::FailedToOpenLockfile(
                        FailedToOpenLockfileError::new(e),
                    )),
                }
            }
            Err(e) => Err(e),
        ...
        if cfg!(target_os = "macos") {
            ...
            return Err(RiftInitializationError::UnknownError(
